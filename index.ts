...
const getPool = (): Promise<mysql2.Pool> => {
  if (!poolPromise) {
    ...
    poolPromise = new Promise<mysql2.Pool>(async (resolve, reject) => {
      try {
        let mysqlConfig = { ...config.mysql };
        
        // If SSH tunneling is enabled, establish the tunnel and update MySQL config
        if (config.ssh.enabled) {
          log('info', 'Establishing SSH tunnel for MySQL connection');
          ...
          // Update MySQL config to connect through the SSH tunnel via local port
          mysqlConfig = {
            ...mysqlConfig,
            host: '127.0.0.1',
            port: localPort,
          ...
          log('info', `MySQL will connect through SSH tunnel on 127.0.0.1:${localPort}`);
        }
...
const createSSHTunnel = (sshConfig: SSHConfig, targetHost: string, targetPort: number): Promise<number> => {
  return new Promise((resolve, reject) => {
    const sshClient = new Client();
    
    sshClient.on('ready', () => {
      // Create a local server that will forward connections to the SSH tunnel
      const server = createServer((socket) => {
        sshClient.forwardOut(
          '127.0.0.1',  // Source address, this can be any valid address
          0,            // Source port, 0 means any available port
          targetHost,   // Destination address (MySQL server)
          targetPort,   // Destination port (MySQL port)
          (err, stream) => {
            if (err) {
              socket.end();
              return;
      ...
      server.listen(0, '127.0.0.1', () => {
        // @ts-ignore - The address() function can return string or AddressInfo
        const address = server.address();
        const port = typeof address === 'string' ? 0 : address?.port || 0;
        
        log('info', `SSH tunnel established on local port ${port}`);
        
        // When the SSH connection closes, also close the server
        sshClient.on('close', () => {
          server.close();
